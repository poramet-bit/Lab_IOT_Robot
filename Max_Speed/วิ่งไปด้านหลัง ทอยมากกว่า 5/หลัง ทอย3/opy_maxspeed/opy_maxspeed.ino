// ============================================================================
// OPY MAX SPEED — LM393 closed-loop odometry (ไฟล์แยก ไม่เกี่ยวกับ opy_20260908121334)
// ----------------------------------------------------------------------------
...
//   8) ROUTE RUN   runLeg + setup/loop
// ----------------------------------------------------------------------------
// ไฟล์นี้ต่างจากตัวเดิม:
//   - ล้อหมุนเต็มกำลังตลอด: PWM 4095, ปิด ramp/ชะลอ, trim = 0 (ค่าเดิมคอมเมนต์ไว้ทุกตัว)
//   - ค่าเส้นทาง LEG1-LEG3 ตั้งเป็น 0 ทั้งหมด ค่าเดิมคอมเมนต์ไว้ท้ายบรรทัด
// พฤติกรรมหลัก:
...
// ============================================================================
// 2) ROUTE — ค่าเส้นทาง แก้ที่นี่ที่เดียว (รูปแบบเดียวกับไฟล์ต้นฉบับ)
// ----------------------------------------------------------------------------
...
// --- LEG 1 ---
const float LEG1_FORWARD_CM = 150;                            // ค่าเดิม: 0 (เคยใช้ 320 / 820)
const float LEG1_SPIN_DEG = 68.5;    const bool LEG1_SPIN_RIGHT = false;  // ค่าเดิม: 89.5 หมุนขวา (ชุดแรกสุด: 563.75 ขวา)
...
// --- LEG 2 ---
const float LEG2_FORWARD_CM = 5000;                              // ค่าเดิม: 400 (ชุดแรกสุด: 1150)
const float LEG2_SPIN_DEG = 74.5;    const bool LEG2_SPIN_RIGHT = true; // ค่าเดิม: 0 (ชุดแรกสุด: 943.75 ซ้าย)
...
// --- LEG 3 ---
const float LEG3_FORWARD_CM = 300;                              // ค่าเดิม: 860
