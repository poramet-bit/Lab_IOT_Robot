...
enum Motion { IDLE, FORWARD, LEFT, RIGHT };
void startMotion(Motion requested);

// Standalone straight drive. Settings copied from robotcurclerun_raw.
// BLE UART: F/START = forward, L = left, R = right, S/STOP = brake + summary.
// Turns use one forward wheel; sensors measure motion without steering correction.
const int ENA = 5;
const int IN1 = 7;
const int IN2 = 6;
const int ENB = 10;
...
int RUN_SPEED = 43750;
const float LEFT_RATIO = 0.9980f;
const int TURN_SPEED = 45000;
...
void startMotion(Motion requested) {
  if (running || braking || report.length() > 0) return;
  noInterrupts();
  pulse_count_L = 0;
  pulse_count_R = 0;
  interrupts();
  ...
  motion = requested;
  pwm_left = requested == LEFT ? 0 : (requested == RIGHT ? TURN_SPEED : static_cast<int>(RUN_SPEED * LEFT_RATIO));
  pwm_right = requested == RIGHT ? 0 : (requested == LEFT ? TURN_SPEED : RUN_SPEED);
  // Stationary wheel is dynamically braked; moving wheel drives forward.
  digitalWrite(IN1, requested == LEFT ? HIGH : LOW);
  digitalWrite(IN2, HIGH);
  digitalWrite(IN3, requested == RIGHT ? HIGH : LOW);
  digitalWrite(IN4, HIGH);
  analogWrite(ENA, requested == LEFT ? 65535 : pwm_left);
  analogWrite(ENB, requested == RIGHT ? 65535 : pwm_right);
...
void buildSummary() {
  ...
  report = "\n=== SUMMARY ===\nMODE=" + String(motion == FORWARD ? "FORWARD" : (motion == LEFT ? "LEFT" : "RIGHT"));
...
void setup() {
  ...
  BLE.advertise();
  Serial.println("Ready: connect BLE UART, subscribe to TX, send F / L / R, then S for summary.");
}
    ...
    if (central && central.connected()) {
      if (rxChar.written()) {
        ...
        if (command == "STOP" || command == "S") {
          brakeMotors();
        } else if (command == "START" || command == "F") {
          startMotion(FORWARD);
        } else if (command == "L") {
          startMotion(LEFT);
        } else if (command == "R") {
